...
```{r}
adata_Full<-read_h5ad("/work/shah/users/Mobina/Project_Squamous/Squamousdata/adata_myeloid_Cancer_cellphonedb.h5ad")

adata_Full<-read_h5ad("/work/shah/users/Mobina/Project_Squamous/Squamousdata/adata_Full_genevector_from_top_with_subpops_with_cancer.h5ad")
...
#celltype frequency
```{r}
a<-as.data.frame(with(adata_Full$obs, signif(table(orig.ident, genevector_l2))))

ggplot(a, aes(x=orig.ident, y=Freq,fill=genevector_l2)) +
  geom_bar(stat="identity",position=position_dodge())+ theme(axis.text.x = element_text(angle = 90))+ ggtitle("Sample-Celltype-Frequency")+xlab("Celltype")  
...
```{R,warning=F}
#adata_Full<-adata_Full[adata_Full$obs$genevector_l2 %in% c("Macrophage","Dendritic","Monocyte","pDC","Cancer")]
...
for(i in 1:nrow(Sample_cell_frequency)){
  ...
  dir.create(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/Sample",Sample_cell_frequency$Var1[i]))
  write_h5ad(anndata = adata_Full_list[[i]],filename=paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/Sample",Sample_cell_frequency$Var1[i],"/Sample",Sample_cell_frequency$Var1[i],".h5ad"))
a<-adata_Full_list[[i]]$obs[,c("index","genevector_l2")]
write.table(x = a,file = paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/Sample",Sample_cell_frequency$Var1[i],"/Sample",Sample_cell_frequency$Var1[i],".tsv"),quote = F,row.names = F,sep = "\t")
  }
```
...
```{r}
sample_dirs <- list.dirs("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb", recursive = F)
...
df_list <- lapply(seq_along(sample_dirs), function(i) {
  ...
  logger::log_info("Processing sample {i} of {length(sample_dirs)}: {sample_name}")
   ...
   sigmeans <- read.table(file = file.path(samp_dir,list.files(path = samp_dir,pattern = "statistical_analysis_significant_means")), header = T, sep = "\t")
  ...
  df$celltype_pair <- gsub(pattern = "B.Cell", replacement = "B_Cell", df$celltype_pair)
  df$celltype_pair <- gsub(pattern = "T.Cell", replacement = "T_Cell", df$celltype_pair)
  df$celltype_pair <- gsub(pattern = "Mast.Cell", replacement = "Mast_Cell", df$celltype_pair)
  df$celltype_pair <- gsub(pattern = "Myeloid.Undefined", replacement = "Myeloid_Undefined", df$celltype_pair)

  cell_counts <- table(read.table(file = file.path(samp_dir,list.files(path = samp_dir,pattern = ".tsv")), header = T, sep = "\t", comment.char = "")$genevector_l2) %>% 
    as.data.frame()
...
```
```{r}
df<-df[which(df$cellA!="Unknown"&df$cellB!="Unknown"),]
```
...
pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/Heatmap_significant.pdf"))  
ggplot(df_heatmap, aes(x =cellA ,
                         y = cellB,
                         fill = Count))+geom_tile()+scale_fill_gradient(low = "white", high = "brown")+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+theme(axis.text.y = element_text(vjust = 1, hjust=1))+
...
pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/Heatmap_significant_no_cancer_cancer.pdf"))  
...
df_heatmap_myeloid<-df_heatmap[which(df_heatmap$cellA=="Cancer"|df_heatmap$cellB=="Cancer"),]
...
p<-ggplot(df_heatmap_myeloid, aes(x=fct_reorder(celltype_pair, Count, median), y=Count,fill=Celltype)) +
  geom_bar(stat="identity")+ theme(axis.text.x = element_text(angle = 90))+xlab("Celltype")   
pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/barplot_Myeloid_Cancer.pdf"))
p
dev.off()


ggplot(df_heatmap_myeloid, aes(x=fct_reorder(cellA, Count, median),
                         y = cellB,
                         fill = Count))+geom_tile()+scale_fill_distiller(palette = "Spectral")+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+theme(axis.text.y = element_text(vjust = 1, hjust=1))+
...
pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/dotplot.pdf"),width = 19,height = 90)
...
pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/dotplot_present_all_patients.pdf"),width = 19,height = 30)
ggplot(data = df_all_patients, aes(celltype_pair, interacting_pair, 
  ...
  ylab("") + 
  xlab("") + 
  ggtitle("Dotplot_cancer_myeloid_interactions_present_in_at_least_5_patients")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size=20))
...
Look at LR pairs that are present in more than 5 patients
```{r}
df_5_patients<-df %>% filter(Count_patients>=5)

pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/dotplot_present_5_patients.pdf"),width = 19,height = 30)
ggplot(data = df_5_patients, aes(celltype_pair, interacting_pair, 
                                      colour = median_sigmeans, size = Count_patients )) + 
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  theme_bw() + 
...
pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/dotplot_patient_specific.pdf"),width = 19,height = 60)
ggplot(data = df_patient_specific, aes(celltype_pair, interacting_pair, 
                                      colour = sigmeans,shape=sample_name)) + 
  geom_point(size=5) +
  scale_color_gradient(low = "blue", high = "red") +scale_shape_manual(values = c(8,10,12,seq(15,19)))+
  theme_bw() + 
  ylab("") + 
  xlab("") + 
...
# pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/dotplot_patient_specific.pdf"),width = 19,height = 90)
...
pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/dotplot_ILRB_family.pdf"),width = 19,height = 10)
...
Cancer.Macrophages
```{r}
df_Cancer_Macrophages_all_patients<-df_all_patients[which(df_all_patients$celltype_pair=="Cancer.Macrophages"|df_all_patients$celltype_pair=="Macrophages.Cancer"),]

pdf(paste0("/work/shah/users/Mobina/Project_Squamous/Results/cellphonedb/Sample_new_version_cellphonedb/dotplot_Cancer_Macrophages.pdf"),width = 19,height = 10)
ggplot(data = df_Cancer_Macrophages_all_patients, aes(celltype_pair, interacting_pair, 
                                      colour = median_sigmeans,size=Count_patients )) + 
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  theme_bw() + 
