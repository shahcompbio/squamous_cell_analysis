...
if (file.exists(deg_file)) {
  ...
  write.table(de.markers.mast, file = deg_file, quote = F, col.names = T, row.names = T, sep = "\t")
}
```
...
de.markers.mast$sig <- ifelse(de.markers.mast$p_val_adj < p_thresh & abs(de.markers.mast$avg_log2FC) >= lfc_thresh, T, F)

de.markers.mast$gene_info <- with(de.markers.mast, factor(ifelse(avg_log2FC > 0, "Up in Tumor", "Down in Tumor")))

de.markers.mast$Gene <- rownames(de.markers.mast)
